...
{
    public JsonSchemaToCSharpResult Generate(string schemaJson, JsonSchemaToCSharpOptions? options = null, CancellationToken cancellationToken = default)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(schemaJson);
        cancellationToken.ThrowIfCancellationRequested();
        options ??= new();
        JsonNode document = JsonNode.Parse(schemaJson) ?? throw new ArgumentException("A schema cannot be null.", nameof(schemaJson));
            ...
            foreach (string keyword in new[] { "definitions", "$defs" })
                if (root[keyword] is JsonObject definitions)
                    foreach ((string key, JsonNode? schema) in definitions.OrderBy(p => p.Key, StringComparer.Ordinal))
                        if (schema != null)
                            types["#/" + keyword + "/" + key.Replace("~", "~0").Replace("/", "~1")] = emitter.GetType(schema, key);
        ...
        JsonSchemaToCSharpOptions? options = null, CancellationToken cancellationToken = default)
    {
        ...
        string json = await File.ReadAllTextAsync(input, cancellationToken).ConfigureAwait(false);
        ...
        foreach ((string relative, string source) in result.Files)
        {
            ...
            if (!path.StartsWith(Path.TrimEndingDirectorySeparator(output) + Path.DirectorySeparatorChar, StringComparison.OrdinalIgnoreCase))
                throw new IOException("Generated path escaped the output directory.");
            if (string.Equals(path, input, StringComparison.OrdinalIgnoreCase)) throw new IOException("Output would overwrite the input schema.");
            if (File.Exists(path) && options?.Overwrite != true) throw new IOException("Output file already exists: " + path);
        ...
        foreach ((string path, string source) in paths)
        {
            ...
            await using var stream = new FileStream(path, options?.Overwrite == true ? FileMode.Create : FileMode.CreateNew,
